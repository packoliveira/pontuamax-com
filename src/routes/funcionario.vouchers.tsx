...
import { Ticket, ShieldOff } from "lucide-react";
...
function Vouchers() {
  const { hasAny } = useEmployeeContext();
  if (!hasAny(["vouchers.validar", "vouchers.criar"])) {
    return (
  ...
  return (
    <div className="max-w-xl space-y-4">
      <h1 className="text-2xl font-bold text-[#0F172A]">Vouchers</h1>
      <Card className="rounded-2xl border-[#E5E7EB]">
        <CardContent className="p-6 space-y-3 text-center">
          ...
          <p className="text-sm text-[#64748B]">Validação e criação de vouchers em breve neste painel.</p>
        </CardContent>
      </Card>
    </div>
  );
