...
// erp currentOrgId removed — org resolved via supabase context
...
const DEFAULT_FIDELITY_PERMS: Perm[] = [
  { id: "perm-fid-1", code: "pode_validar_voucher", name: "Validar Vouchers / QR Code", description: "Escanear e dar baixa em prêmios e vouchers no balcão", module: "fidelidade" },
  { id: "perm-fid-2", code: "pode_pontuar_compra", name: "Pontuar Compra no Caixa", description: "Inserir CPF e creditar pontos/cashback aos clientes", module: "fidelidade" },
  { id: "perm-fid-3", code: "pode_estornar_transacao", name: "Estornar Transações", description: "Cancelar pontuações ou resgates no caixa", module: "fidelidade" },
  { id: "perm-fid-4", code: "pode_dar_desconto_manual", name: "Ajuste Manual de Saldo", description: "Conceder descontos ou créditos manuais de saldo", module: "fidelidade" },
  { id: "perm-fid-5", code: "pode_ver_historico_caixa", name: "Ver Histórico do Turno", description: "Visualizar histórico completo de movimentações do caixa", module: "fidelidade" },
];
...
function CargosPage() {
  ...
  const perms = useQuery({
    ...
    queryFn: async () => {
      ...
      const existingCodes = new Set(list.map(p => p.code));
      for (const fp of DEFAULT_FIDELITY_PERMS) {
        if (!existingCodes.has(fp.code)) {
          list.push(fp);
        }
      }
      return list;
  ...
  async function togglePerm(permId: string, on: boolean) {
    if (!selected) return;
    let targetPermId = permId;
    if (permId.startsWith("perm-fid-")) {
      const fp = DEFAULT_FIDELITY_PERMS.find(p => p.id === permId);
      if (fp) {
        const { data: dbP } = await supabase.from("permissions").select("id").eq("code", fp.code).maybeSingle();
        if (dbP?.id) {
          targetPermId = dbP.id;
        } else {
          const { data: newP, error: pErr } = await supabase.from("permissions").insert({
            code: fp.code,
            name: fp.name,
    ...
    if (on) {
      const { error } = await supabase.from("role_permissions")
        .upsert({ role_id: selected.id, permission_id: targetPermId, allowed: true });
      if (error) return toast.error(error.message);
    } else {
      const { error } = await supabase.from("role_permissions")
        .delete().eq("role_id", selected.id).eq("permission_id", targetPermId);
      if (error) return toast.error(error.message);
    }
