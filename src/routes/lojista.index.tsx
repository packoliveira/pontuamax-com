...
function Dashboard() {
  ...
  const stats: { label: string; value: string | number; icon: LucideIcon; show: boolean; hint?: string }[] = [
    { label: "Clientes", value: clientes.length.toLocaleString("pt-BR"), icon: Users, show: true, hint: "Total cadastrados" },
    { label: "Pontos no mês", value: pontosMes.toLocaleString("pt-BR"), icon: Coins, show: inclP, hint: "Distribuídos" },
    { label: "Cashback do mês", value: formatBRL(cashbackMes), icon: Wallet, show: inclC, hint: "Devolvido a clientes" },
    { label: "Resgates pendentes", value: pendentes, icon: Gift, show: true, hint: "Aguardando validação" },
  ].filter((s) => s.show);
      ...
      <Card className="rounded-2xl border border-[#E5E7EB] bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
        <CardContent className="p-6">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link to="/lojista/clientes" className="text-xs font-medium text-[#2563EB] hover:underline">
              Ver clientes
            </Link>
          </div>
          {txs.length === 0 ? (
            <div className="rounded-xl border border-dashed border-[#E5E7EB] bg-[#F8FAFC] p-8 text-center">
              ...
              <Link to="/lojista/lancar-venda">
                <Button variant="outline" className="mt-4 rounded-xl border-[#E5E7EB] text-[#2563EB] hover:bg-[#2563EB]/5">
                ...
                return (
                  <li key={t.id} className="flex items-center justify-between gap-3 py-3 text-sm">
                    ...
                    <div className="shrink-0 text-right">
                      {t.tipo === "venda" && <div className="font-semibold text-[#0F172A]">{formatBRL(Number(t.valor))}</div>}
                      {t.pontos_delta ? (
                        <div className={`text-xs font-medium ${t.pontos_delta > 0 ? "text-[#16A34A]" : "text-[#EF4444]"}`}>
                          {t.pontos_delta > 0 ? "+" : ""}
                          {t.pontos_delta} pts
                        </div>
                      ) : null}
                      {Number(t.cashback_delta) ? (
                        <div className={`text-xs font-medium ${Number(t.cashback_delta) > 0 ? "text-[#16A34A]" : "text-[#EF4444]"}`}>
