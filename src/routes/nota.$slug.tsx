...
function Page() {
  ...
  const submit = useMutation({
    ...
    onSuccess: () => { toast.success("Nota enviada! Aguarde aprovação do lojista."); qc.invalidateQueries({ queryKey: ["my-fiscal-notes"] }); },
  ...
  const onFile = async (f: File | null) => {
    ...
    try {
      ...
      const b64 = btoa(bin);
      await submit.mutateAsync({ data: { store_id: loja.id, image_path: path, image_base64: b64, mime: f.type || "image/jpeg" } });
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6">
      <div className="max-w-lg mx-auto space-y-4">
        ...
        <Card>
          <CardHeader><CardTitle className="text-base">Tire uma foto legível da nota</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            <input ref={inputRef} type="file" accept="image/*" capture="environment" hidden onChange={(e) => onFile(e.target.files?.[0] ?? null)} />
            <Button className="w-full" disabled={busy || submit.isPending} onClick={() => inputRef.current?.click()}>
              <Upload className="h-4 w-4 mr-2" />{busy || submit.isPending ? "Enviando..." : "Escolher foto"}
            </Button>
            <p className="text-xs text-muted-foreground">A IA extrai valor e CNPJ automaticamente. O lojista aprova e os pontos caem na sua conta.</p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader><CardTitle className="text-base">Suas notas</CardTitle></CardHeader>
          <CardContent className="space-y-2">
            {minhasNotas.length === 0 ? <p className="text-sm text-muted-foreground">Nenhuma ainda.</p> : minhasNotas.map((n) => (
              <div key={n.id} className="flex items-center justify-between text-sm border rounded-md p-2">
                <span>R$ {n.valor?.toFixed(2) ?? "?"} · {new Date(n.created_at).toLocaleDateString("pt-BR")}</span>
                <Badge variant={n.status === "aprovada" ? "default" : n.status === "rejeitada" ? "destructive" : "secondary"}>
                  {n.status}{n.pontos_creditados ? ` · +${n.pontos_creditados}` : ""}
                </Badge>
              </div>
            ))}
          </CardContent>
