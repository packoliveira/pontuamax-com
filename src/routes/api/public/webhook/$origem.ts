    ...
    handlers: {
      ...
      POST: async ({ request, params }) => {
        ...
        if (!Number.isFinite(valor) || valor <= 0) {
          // Olist envia notificações leves sem o total no payload.
          // Cliente já foi vinculado como pendente — aguardamos uma
          // próxima notificação do mesmo pedido que traga o valor.
          return logAndRespond(
            "erro",
