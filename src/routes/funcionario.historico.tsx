          ...
          <div className="divide-y divide-[#F1F5F9]">
            {(txs as any[]).length === 0 ? (
              ...
              <div key={t.id} className="p-3 flex items-center gap-3 text-sm">
                <div className="text-[11px] text-[#64748B] w-32 shrink-0">{new Date(t.created_at).toLocaleString("pt-BR")}</div>
                <div className="flex-1 min-w-0">
                  <div className="font-semibold text-[#0F172A]">{t.tipo}</div>
                  <div className="text-xs text-[#64748B] truncate">{t.profiles?.full_name ?? "—"}</div>
                </div>
                <Badge variant="secondary" className="rounded-full">
                  {t.pontos_delta ? `${t.pontos_delta > 0 ? "+" : ""}${t.pontos_delta} pts` : ""}
                  {t.cashback_delta ? ` ${Number(t.cashback_delta) > 0 ? "+" : ""}R$ ${Number(t.cashback_delta).toFixed(2)}` : ""}
                </Badge>
              </div>
            ))}
          </div>
