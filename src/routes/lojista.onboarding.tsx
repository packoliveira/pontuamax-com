...
function Onboarding() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        prefill = sessionStorage.getItem("onboarding_prefill_email");
      } catch {
        /* ignore */
      }
      const { data } = await supabase.auth.getSession();
      ...
      try {
        sessionStorage.removeItem("onboarding_prefill_email");
      } catch {
        /* ignore */
      }
    })();
  ...
  const finalizar = async () => {
    ...
    try {
      if (!alreadyAuthed) {
        ...
        if (sErr) {
          if (isUsuarioJaCadastrado(sErr)) {
            setErroEmail(
              "Esse email já está cadastrado. Volte para a tela de login e entre com sua senha.",
            );
            setStep(1);
          }
          throw sErr;
      ...
      try {
        ...
        if (bannerMobileFile)
          uploadedUrls.banner_url_mobile = await doUpload(bannerMobileFile, "banner-mobile");
        ...
        toast.warning(
          "Loja criada, mas houve um problema no upload das imagens. Você pode enviá-las depois em Configurações.",
        );
        console.error(upErr);
      }
      setSlug(s);
    ...
    <div className="min-h-screen bg-gradient-to-br from-violet-50 to-orange-50 p-4 py-8">
      <div className="max-w-3xl mx-auto space-y-4">
        <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground">
          <span>
            Passo {alreadyAuthed ? step - 1 : step} de {alreadyAuthed ? 3 : 4}
          </span>
          {alreadyAuthed && email && (
        ...
        {step === 1 && !alreadyAuthed && (
          <Card>
            <CardHeader>
              <CardTitle>Sua conta de lojista</CardTitle>
            </CardHeader>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={() => setStep(3)} disabled={!nome} className="flex-1">
                  Próximo <ArrowRight className="h-4 w-4" />
                </Button>
              </div>
            </CardContent>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={finalizar} disabled={loading} className="flex-1">
            ...
            <CardContent className="pt-6 text-center space-y-4">
              ...
              <div className="flex items-center gap-2 max-w-md mx-auto rounded-md border bg-muted p-2">
                ...
                <Button
                  ...
                  variant="ghost"
                  onClick={() => {
                    navigator.clipboard.writeText(`pontuamax.com.br/${slug}`);
              ...
              <div className="rounded-lg border bg-amber-50 border-amber-200 p-3 text-sm text-amber-900 text-left">
                ...
                <p className="text-xs mt-1">
                  Sua loja foi criada mas o painel só é liberado após o pagamento da implementação.
                  Nossa equipe entrará em contato.
                </p>
              </div>
              <Button
                onClick={() => navigate({ to: "/lojista/aguardando" })}
    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-3">
        {localUrl ? (
          ...
          <div
            className={`${cls} rounded-md border border-dashed flex items-center justify-center text-xs text-muted-foreground`}
          >
